...
The hero statement uses Newsreader, matching the English name. The Chinese name uses Adobe's [Source Han Serif SC Medium 2.003](https://github.com/adobe-fonts/source-han-serif/tree/2.003R/OTF/SimplifiedChinese), self-hosted as a WOFF2 subset containing only 吴昊阳. The subset is renamed `Haoyang Name Serif` to respect the reserved font name; its original glyph outlines are preserved. The upstream OFL notice is included as `assets/academic/fonts/Source-Han-Serif-OFL.txt`.
